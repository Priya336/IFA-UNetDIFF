...
def my_psnr(X,Y):
    ...
    psnr = 0.0
    for i in range(ch):
        mse = np.mean(np.power(X[:,:,i] - Y[:,:,i], 2))
        # avoid log10 of zero
        if mse <= 0:
            continue
        psnr = psnr + 10*log10(1.0 / mse)
    return psnr/ch

# --- New metric helpers ---

def sam_metric(gt, pred, eps=1e-12):
    """
    Compute mean Spectral Angle Mapper (radians) between gt and pred.
    gt, pred: H x W x C (numpy), assumed aligned and same dtype
    returns mean_sam_radians
    """
    H, W, C = gt.shape
    gt_flat = gt.reshape(-1, C)  # (N, C)
    pr_flat = pred.reshape(-1, C)
    # dot and norms
    dot = np.sum(gt_flat * pr_flat, axis=1)
    n1 = np.linalg.norm(gt_flat, axis=1)
    n2 = np.linalg.norm(pr_flat, axis=1)
    denom = (n1 * n2) + eps
    cosval = np.clip(dot / denom, -1.0, 1.0)
    angles = np.arccos(cosval)
    return np.mean(angles)

def scc_metric(gt, pred):
    """
    Compute SCC as band-wise Pearson correlation between GT and Pred;
    average across bands. Values in [-1,1], higher better.
    gt, pred: H x W x C
    """
    H, W, C = gt.shape
    sccs = []
    for b in range(C):
        g = gt[:,:,b].flatten()
        p = pred[:,:,b].flatten()
        # if constant, correlation undefined -> set to 0
        if np.allclose(g, g.mean()) or np.allclose(p, p.mean()):
            sccs.append(0.0)
        else:
...
def q_index_per_band(gt, pred, eps=1e-12):
    """
    Compute single-band Q index (Wang & Bovik) between gt and pred.
    Returns scalar Q in [-1,1], higher is better.
    """
    ...
    var_g = gt.var()
    var_p = pred.var()
    cov = ((gt - mean_g)*(pred - mean_p)).mean()
    num = 4.0 * cov * mean_g * mean_p
    den = (var_g + var_p) * (mean_g**2 + mean_p**2) + eps
    return num / den

def q2n_approx(gt, pred):
    """
    Approximate multiband quality by averaging single-band Q indices.
    (This is an approximation to the true multivariate Q2n.)
    """
    H, W, C = gt.shape
    qs = []
    for b in range(C):
        q = q_index_per_band(gt[:,:,b], pred[:,:,b])
        qs.append(q)
    return np.mean(qs)

def ergas_metric(gt, pred, scale):
    """
    Compute ERGAS: 100 * scale * sqrt( (1/N) * sum_b (MSE_b / mean_b^2) )
    gt, pred: H x W x C
    scale: spatial scale factor (e.g., 4)
    """
    H, W, C = gt.shape
    mse_b = np.zeros(C, dtype=np.float64)
    mean_b = np.zeros(C, dtype=np.float64)
    for b in range(C):
        g = gt[:,:,b]
        p = pred[:,:,b]
        mse_b[b] = np.mean((g - p)**2)
        mean_b[b] = np.mean(g)
    # prevent division by zero
    denom = mean_b**2
    denom[denom == 0] = 1e-12
    val = np.mean(mse_b / denom)
    ergas = 100.0*(1.0/scale)*np.sqrt(val)
    return ergas
...
if __name__ == "__main__":
    ...
    gt = HRHS.squeeze(0).permute(1,2,0).numpy() / nf
    pred = im_out / nf

    psnr = my_psnr(gt, pred)

    ssim = SSIM(gt, pred, data_range=1)

    # --- NEW METRICS ---
    sam_rad = sam_metric(gt, pred)               # radians (mean)
    sam_deg = sam_rad * 180.0 / np.pi            # degrees

    scc = scc_metric(gt, pred)                   # average band-wise Pearson corr

    q2n_approx_val = q2n_approx(gt, pred)        # band-averaged Q (approx Q2n)

    ergas = ergas_metric(gt, pred, scale=opt['scale'])

    ## save output
    sio.savemat(join(out_path, dname+"_"+args.res+"_Ours_unet_.mat"), {'R_Ours': im_out,'E':E, 'A':A})

    print(f"{dname:s} \t PSNR: \t {psnr:.2f} \t SSIM: \t {ssim:.4f} \t Time: {Ours_time:.2f}")
    print(f"SAM (mean): {sam_rad:.6f} rad  / {sam_deg:.4f} deg")
    print(f"SCC (avg band-wise Pearson): {scc:.4f}")
    print(f"Q2n (approx, band-averaged Q): {q2n_approx_val:.4f}  [approximation]")
